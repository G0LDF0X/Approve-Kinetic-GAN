...
parser.add_argument("--data_path", type=str, default="/content/drive/MyDrive/xsub/train_data.npy", help="path to data")
parser.add_argument("--label_path", type=str, default="/content/drive/MyDrive/xsub/train_label.pkl", help="path to label")
...
def quantile_masking(quantile, original, noised):
  sub_matrix = torch.abs(original - noised)
  quantile_threshold = torch.quantile(sub_matrix, quantile)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # 4. Quantil Masking을 이용한 Stochastic Variation 효과 향상
        # z2 = Tensor(np.random.normal(0, 0.01, (opt.batch_size, opt.latent_dim)))
        # z3 = z + z2

        # Generate a batch of actions
        fake_imgs = generator(z, labels)
        # noised_fake_imgs = generator(z3, labels)
        # masking_matrix = quantile_masking(0.5, fake_imgs, noised_fake_imgs)
        # quntile_fake_imgs = masking_matrix * np.random.normal(0, 1, 1)[0] + 1
