...
class Discriminator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        N, C, T, V = x.size() # 32, 3, 64, 25
        
        c = self.label_emb(labels)
        
        # 3. Dicriminator의 label에 noise를 주입하여 label smoothing
        # noise = torch.Tensor(np.random.normal(0, 1, (N, 60))).cuda()
        # noised_c = c + noise

        c = c.view(c.size(0), c.size(1), 1, 1).repeat(1, 1, T, V)
        # noised_c = c.view(c.size(0), c.size(1), 1, 1).repeat(1, 1, T, V)
...
class st_gcn(nn.Module):
    ...
    def downsample_s(self, tensor):
        ...
        # 2. 다운샘플링에 가우시안 노이즈 추가
        # noise = torch.ones_like(tensor[:,:,:,keep]) * np.random.normal(0, 1, 1)[0]
        # result = noise + tensor[:,:,:,keep]
